...
<style>
  :root{
    --green:#02B140; --green-dark:#164734; --green-soft:#E4F6E9;
    --ink:#0E151C; --sub:#71757A; --line:#ECECEC; --bg:#F5F6F7; --card:#fff;
  ...
  *{box-sizing:border-box; -webkit-tap-highlight-color:transparent}
  ...
  /* calendar strip */
  .cal{display:flex; gap:7px; overflow-x:auto; padding:6px 2px 12px; scrollbar-width:none}
  .cal::-webkit-scrollbar{display:none}
  .cal-day{flex:0 0 auto; width:48px; padding:9px 0; border-radius:13px; text-align:center;
    background:var(--card); border:1.5px solid var(--line); cursor:pointer; transition:.12s}
  .cal-day .dn{font-size:17px; font-weight:800; color:var(--ink)}
  .cal-day .wd{font-size:10px; font-weight:600; color:var(--sub); margin-top:2px; text-transform:uppercase}
  .cal-day.has{border-color:#CDEAC4}
  .cal-day.has .dn{color:var(--green-dark)}
  .cal-day.active{background:var(--green); border-color:var(--green)}
  .cal-day.active .dn,.cal-day.active .wd{color:#fff}
  .cal-day.alldays{width:auto; padding:9px 14px; display:flex; align-items:center}
  .cal-day.alldays .dn{font-size:13px; font-weight:700}
  ...
  .ecard .img{aspect-ratio:5/7; background:linear-gradient(135deg,#02B140,#164734); position:relative;
    display:flex; align-items:center; justify-content:center}
  .ecard .img img{width:100%; height:100%; object-fit:cover}
  .ecard .img .ph{color:rgba(255,255,255,.4)}
  .ecard .body{padding:11px 12px}
  .ecard .ttl{font-size:14px; font-weight:800; color:var(--ink); line-height:1.25}
  .ecard .meta-date{font-size:13px; margin-top:5px}
  .ecard .meta-date b{font-weight:800; color:var(--ink)}
  .ecard .meta-date span{color:var(--sub)}
  .ecard .meta-place{font-size:12px; color:var(--sub); margin-top:2px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis}
  ...
  .evpage{background:var(--card); border:1px solid var(--line); border-radius:16px; overflow:hidden; margin-bottom:12px}
  .evpage-img{aspect-ratio:5/7; max-height:420px; background:linear-gradient(135deg,#02B140,#164734); display:flex; align-items:center; justify-content:center}
  .evpage-img img{width:100%; height:100%; object-fit:cover}
  .evpage-img .ph{color:rgba(255,255,255,.4)}
  .evpage-body{padding:18px 16px}
  .evpage-ttl{margin:0 0 12px; font-size:22px; font-weight:800; line-height:1.2; color:var(--ink)}
  .evpage-date{font-size:18px; margin-bottom:14px}
  .evpage-date b{font-weight:800; color:var(--ink)}
  .evpage-date span{color:var(--sub)}
  .evpage-row{display:flex; justify-content:space-between; align-items:center; gap:10px;
    padding:12px 0; border-top:1px solid var(--line)}
  .evpage-label{font-size:13px; font-weight:700; color:var(--sub)}
  .evpage-place{font-size:15px; font-weight:600; color:var(--ink); text-align:right}
  .reg-card{background:var(--card); border:1px solid var(--line); border-radius:16px; padding:16px; margin-bottom:90px}
  ...
  .vnav{position:fixed; left:0; right:0; bottom:0; display:flex; gap:8px; max-width:460px; margin:0 auto;
    padding:10px 16px; background:var(--card); border-top:1px solid var(--line); z-index:15}
  .vnav-btn{flex:1; padding:11px 0; border-radius:12px; font-size:14px; font-weight:700; font-family:inherit;
    border:none; background:#E7E9EB; color:var(--sub); cursor:pointer}
  .vnav-btn.on{background:var(--green); color:#fff}
...
function setHeaderSub(){
  if(state.role==='volunteer' && me){
    headerSub.innerHTML = `<button id="hdr-profile" style="display:flex;align-items:center;gap:6px;background:none;border:1.5px solid var(--line);border-radius:999px;padding:6px 12px 6px 10px;cursor:pointer;font-family:inherit;font-weight:700;font-size:13px;color:var(--ink)">
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="8" r="3.4" stroke="currentColor" stroke-width="1.8"/><path d="M5 19c0-3.3 3-5.5 7-5.5s7 2.2 7 5.5" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/></svg>
      Профиль</button>`;
    const b=document.getElementById('hdr-profile');
    if(b) b.onclick=()=>{ vol.vtab='profile'; vol.view='list'; render(); };
  } else if(state.role==='coordinator'){
    headerSub.textContent = 'Координатор';
  } else {
...
// нижние вкладки волонтёра
function volNav(){
  return `<div class="vnav">
    <button class="vnav-btn ${vol.vtab==='events'?'on':''}" id="nav-events">События</button>
    <button class="vnav-btn ${vol.vtab==='profile'?'on':''}" id="nav-profile">Профиль</button>
  </div>`;
}
function bindVolNav(){
  const e=document.getElementById('nav-events'), p=document.getElementById('nav-profile');
  if(e) e.onclick=()=>{ vol.vtab='events'; vol.view='list'; render(); };
  if(p) p.onclick=()=>{ vol.vtab='profile'; render(); };
}
...
function renderList(){
  ...
  let calHtml = '';
  if(dates.length){
    calHtml = `<div class="cal">
      <div class="cal-day alldays ${vol.day==='all'?'active':''}" data-day="all"><span class="dn">Все дни</span></div>` +
      dates.map(d=>{
        const [y,m,day]=d.split('-'); const wd=WD[new Date(d).getDay()];
        return `<div class="cal-day has ${vol.day===d?'active':''}" data-day="${d}">
          <div class="dn">${+day}</div><div class="wd">${wd}</div></div>`;
      }).join('') + `</div>`;
  }
  const q = (vol.q||'').trim().toLowerCase();
  let shown = state.events.filter(e=> vol.day==='all' || e.date===vol.day);
  ...
  const evCards = shown.map(e=>{
    const taken=+e.taken||0, full=taken>=e.need, mine=signedIds.has(e.id);
    return `<button class="ecard" data-id="${esc(e.id)}">
      <div class="img">${eventImg(e)}</div>
      <div class="body">
        <div class="ttl">${esc(e.title)}</div>
        ${e.date?`<div class="meta-date"><b>${fmtDateFull(e.date)}</b><span>, ${weekday(e.date)}</span></div>`:''}
        ${e.place?`<div class="meta-place">${esc(e.place)}</div>`:''}
        <div class="foot">
          ${mine?'<span class="pill green">✓ Вы записаны</span>':(full?'<span class="pill amber">Мест нет</span>':`<span class="pill green">${taken}/${e.need}</span>`)}
        </div>
      </div>
    </button>`;
  }).join('');

  el.innerHTML = `<div class="hello">Привет, ${esc(me.name.split(' ')[0])}!</div>` + calHtml +
    `<div class="search-wrap"><input id="ev-search" class="search" placeholder="Поиск события…" value="${esc(vol.q)}"></div>
    <div class="section-label">Выбери событие:</div>
    <div class="egrid">${shown.length?evCards:'<div class="card center muted" style="grid-column:1/-1">Ничего не найдено.</div>'}</div>` + volNav();
  ...
  if(si) si.oninput=e=>{ vol.q=e.target.value; const grid=document.querySelector('.egrid'); 
    ...
    let f=state.events.filter(x=> vol.day==='all'||x.date===vol.day);
    if(qq) f=f.filter(x=>(x.title||'').toLowerCase().includes(qq)||(x.place||'').toLowerCase().includes(qq));
    grid.innerHTML = f.length? f.map(ev=>{const taken=+ev.taken||0,full=taken>=ev.need,mine=signedIds.has(ev.id);
      return `<button class="ecard" data-id="${esc(ev.id)}"><div class="img">${eventImg(ev)}</div><div class="body"><div class="ttl">${esc(ev.title)}</div>${ev.date?`<div class="meta-date"><b>${fmtDateFull(ev.date)}</b><span>, ${weekday(ev.date)}</span></div>`:''}${ev.place?`<div class="meta-place">${esc(ev.place)}</div>`:''}<div class="foot">${mine?'<span class="pill green">✓ Вы записаны</span>':(full?'<span class="pill amber">Мест нет</span>':`<span class="pill green">${taken}/${ev.need}</span>`)}</div></div></button>`;
    }).join('') : '<div class="card center muted" style="grid-column:1/-1">Ничего не найдено.</div>';
...
function renderEventPage(){
  ...
  el.innerHTML = `
    ...
    <div class="evpage">
      <div class="evpage-img">${eventImg(e)}</div>
      <div class="evpage-body">
        <h1 class="evpage-ttl">${esc(e.title)}</h1>
        ${e.date?`<div class="evpage-date"><b>${fmtDateFull(e.date)}</b><span>, ${weekday(e.date)}</span></div>`:''}
        ${e.place?`<div class="evpage-row"><span class="evpage-label">Место</span><div class="evpage-place">${esc(e.place)}</div></div>`:''}
        <div class="evpage-row"><span class="evpage-label">Волонтёры</span>
          <div>${full?'<span class="pill amber">Мест нет</span>':`<span class="pill green">Записано ${taken} из ${e.need}</span>`}</div>
        </div>
      </div>
    </div>
    ${already
      ? `<div class="card center"><p class="muted" style="margin:6px 0">Вы уже записаны на это событие. Статус: <b style="color:var(--ink)">${already.status==='approved'?'подтверждён':'ждёт подтверждения'}</b>.</p></div>`
      : `<div class="reg-card">
          <h3 class="reg-ttl">Записаться волонтёром</h3>
          <p class="muted" style="margin:-6px 0 0">Запишем как <b style="color:var(--ink)">${esc(me.name)}</b> (${esc(me.phone)})</p>
          <div class="err" id="v-err" style="display:none;margin-top:10px"></div>
        </div>`}
    ${already?'':`<div class="sticky-bar">
      <button class="btn" id="v-submit" ${full?'disabled':''}>${full?'Мест больше нет':'Записаться'}</button>
    </div>`}`;

  document.getElementById('back').onclick=()=>{ vol.view='list'; render(); };
  const sub=document.getElementById('v-submit');
  if(sub) sub.onclick=async()=>{
    // спросим разрешение на уведомления при первой записи (если поддерживается)
...
function renderDone(){
  ...
  el.innerHTML = `<div class="card center" style="margin-top:20px">
    ...
    <button class="btn" id="again" style="margin-top:8px">К списку событий</button>
  </div>`;
...
function renderProfile(){
  const cards = vol.mySignups.map(s=>{
    ...
    return `<div class="card">
      ...
      <div style="margin-top:8px"><span class="pill ${appr?'green':'amber'}">${appr?'Подтверждён':'Ждёт подтверждения'}</span></div>
      ${appr && (s.scan_login||s.scan_pass) ? `<div class="scan-box">
      ...
      </div>` : (appr?'<div class="muted" style="margin-top:8px;font-size:13px">Доступ к сканеру координатор добавит позже.</div>':'')}
    </div>`;
  }).join('');
  ...
  el.innerHTML = `
    <div class="card" style="margin-top:8px">
      ...
      <button class="btn ghost small" id="logout" style="margin-top:12px">Выйти</button>
    </div>
...
// ---------- COORDINATOR ----------
let coord = { tab:'people', filter:'all', err:'', editing:null, editingVol:null, q:'', limit:20, teamTab:'volunteers', teamQ:'', viewUser:null };
...
function drawCoordinator(){
  ...
  if(coord.tab==='people'){
    ...
    let visible = s.filter(x=> coord.filter==='all'? x.status!=='rejected' : x.status===coord.filter);
    ...
    document.getElementById('sync').onclick=async()=>{
      ...
      state.events=await api.eventsCoord(); state.signups=await api.listSignups();
    ...
    document.querySelectorAll('[data-save]').forEach(b=>b.onclick=async()=>{
      ...
      if(!title){ return; }
      await api.addEvent({id,title,date:date||null,place:place||null,need,scan_login:scan_login||null,scan_pass:scan_pass||null});
    ...
    document.getElementById('e-add').onclick=async()=>{
      ...
      if(!title){ return; }
      await api.addEvent({title,date:date||null,place:place||null,need,scan_login:scan_login||null,scan_pass:scan_pass||null}); state.events=await api.eventsCoord(); drawCoordinator();
    };
  ...
  if(coord.teamTab==='coords'){
    ...
    function renderList(){
      ...
      listEl.innerHTML = list.length ? list.map(v=>`
        <div class="card" style="display:flex;justify-content:space-between;align-items:center;gap:8px">
          <button class="user-open" data-uphone="${esc(v.phone)}" style="flex:1;text-align:left;background:none;border:none;cursor:pointer;font-family:inherit;padding:0">
            <div style="font-weight:800;font-size:16px;color:var(--ink)">${esc(v.name)}</div>
            <div class="muted">${esc(v.phone)}</div>
          </button>
          <div style="display:flex;gap:6px;flex:0 0 auto">
            <button class="icon-btn" data-uedit="${esc(v.phone)}" title="Редактировать">✏️</button>
            <button class="icon-btn" data-udel="${esc(v.phone)}" title="Удалить">🗑️</button>
          </div>
      ...
      listEl.querySelectorAll('[data-uedit]').forEach(b=>b.onclick=async()=>{
        const phone=b.dataset.uedit; const v=vols.find(x=>x.phone===phone);
        const nn=prompt('Имя волонтёра:', v?v.name:''); if(nn===null) return;
        await api.patchVolunteer({phone,name:nn.trim()}); drawCoordinator();
      });
...
(async function init(){
  ...
  if(coordAuth){ state.role='coordinator'; render(); return; }
